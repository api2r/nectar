...
For Crossref, the actual work items live at `message$items` in the response body. You can attach `resp_tidy_json()` to the request via the `tidy_policy` argument in `req_prepare()`, so that every response is automatically tidied when you call `resp_parse()` later:
