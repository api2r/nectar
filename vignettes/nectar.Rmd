...
The Crossref API returns JSON. nectar's `resp_tidy_json_tibblify()` function parses the JSON response body and converts the result to a tibble using `tibblify::tibblify()`. You can extract a nested path from the response at the same time with the `subset_path` argument.

For Crossref, the actual work items live at `message$items` in the response body. To have `req_prepare()` automatically tidy each response when you call `resp_parse()` later, supply a prepared tidying policy object via the `tidy_policy` argument, such as `tidy_policy_json_tibblify(subset_path = c("message", "items"))`:
...
`resp_parse()` converts the raw responses into a usable R object. Because the request was prepared with `tidy_policy = tidy_policy_json_tibblify(...)`, `resp_parse()` will find that function automatically and apply it to each response, then combine the results:
